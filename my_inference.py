...
def main():
    ...
    parser.add_argument('--ckpt', default='/data/home/xueyao/workspace/HierSpeechpp/hierspeechpp_eng_kor/hierspeechpp_v1.1_ckpt.pth')
    ...
    hps = utils.get_hparams_from_file(os.path.join(os.path.split(args.ckpt)[0], 'config.json'))
    h_sr = utils.get_hparams_from_file(os.path.join(os.path.split(args.ckpt_sr)[0], 'config.json') )
    h_sr48 = utils.get_hparams_from_file(os.path.join(os.path.split(args.ckpt_sr48)[0], 'config.json') )
    hps_denoiser = utils.get_hparams_from_file(os.path.join(os.path.split(args.denoiser_ckpt)[0], 'config.json'))

    global hierspeech
    hierspeech = model_load(args) 
...
if __name__ == '__main__':
    ...
    output_root = "/data/home/xueyao/workspace/HierSpeechpp/vc_results_my"
    model_name = "hierspeechpp"
    ...
    for group, content_reference_paths_pairs in group_dict.items():
        ...
        for cont_path, ref_path in tqdm(content_reference_paths_pairs):
            ...
            # Inference
            args.source_speech = cont_path
            args.target_speech = ref_path
            args.output_file = conversion_path
            inference(args)
